...
if active_tab == tabs[0]:
    ...
    if st.button("🚀 เริ่มสร้างรายงาน", type="primary", use_container_width=True):
        if report_uploaded_files and company_name:
            ...
            with st.spinner("กำลังวิเคราะห์เอกสารและสร้างรายงานฉบับสมบูรณ์... (ขั้นตอนนี้อาจใช้เวลาหลายนาที)"):
                try:
                    # Send company_name along with the files
                    response = requests.post(
                        REPORT_GEN_URL, 
                        files=files_to_upload, 
                        data={"company_name": company_name}, # <-- ส่งชื่อบริษัทไปด้วย
                        timeout=1200
                    ) 
